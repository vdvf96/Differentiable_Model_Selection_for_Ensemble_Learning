...
# JK modded for efficiency on GPU
def sample_noise_with_gradients_special(noise, shape, input_type, device=None):
    """Samples a noise tensor according to a distribution with its gradient.
    ...
    if noise == _GUMBEL:
        sampler = Gumbel(torch.tensor(0,dtype=input_type).to(device=device), torch.tensor(1,dtype=input_type).to(device=device))
        samples = sampler.sample(shape)
        gradients = 1 - torch.exp(-samples)
    elif noise == _NORMAL:
        sampler = Normal(torch.tensor(0,dtype=input_type).to(device=device), torch.tensor(1,dtype=input_type).to(device=device))
        samples = sampler.sample(shape)
        gradients = samples
...
def perturbed_special(func=None,
                      num_samples = 1000,
                      sigma = 0.05,
                      noise = _NORMAL,
                      batched = True,
...
def perturbed_extended(func=None,
                       num_samples = 1000,
                       sigma = 0.05,
                       noise = _NORMAL,
                       batched = True,
    ...
    def wrapper(input_tensor, *args):
        class PerturbedFunc(torch.autograd.Function):
            ...
            def backward(ctx, dy):
                ...
                dy_save = dy
                input_tensor_save = input_tensor

                g_avg = 0
                for k in range(noise_chunks):

                    dy = dy_save
                    input_tensor = input_tensor_save
